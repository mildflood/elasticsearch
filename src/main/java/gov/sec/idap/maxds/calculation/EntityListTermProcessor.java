...
import gov.sec.idap.maxds.service.EntityService;
import gov.sec.idap.maxds.service.NormalizedFactService;
import gov.sec.idap.maxds.service.ProcessingLogService;
...
public class EntityListTermProcessor {
    ...
    public Boolean process() {
        ...
        cikNumericIdsAsString
                = NormalizedFactService.buildEntitySolrQueryStringForFacts(myEntityList);

        entityIdsQueryString
                = NormalizedFactService.buildEntitySolrQueryStringForFilings(myEntityList);
    ...
    private void saveTermResults() {
        ...
        for (TermResultsDoc tr : termResults) {
            String key = String.format("%s %s %d", tr.entityId, tr.FQ.toString(), tr.FY);
...
//                tr.otherTermResults.add(current);
    ...
    private String buildTermResultkey(TermResultsDoc tr) {
        return String.format("%s %s %d", tr.entityId, tr.FQ, tr.FY);
    }
    ...
    private List<NormalizedFact> filterFactsForIFRSStandard(List<NormalizedFact> facts, String elementName) {
        ...
        for (NormalizedFact nf : facts) {
            ...
            if( nf.elementType.equals("monetary") && ! nf.num_uom.equals("USD")){
                continue;
            }
            
            if (nf.accountingStandard == null || nf.accountingStandard.isEmpty()) {
                filteredFacts.add(nf);
            } else {
                if (nf.accountingStandard.equals("ifrs")) {
                    if (isIFRS) {
                        filteredFacts.add(nf);
                    }
                } else {
    ...
    private void processConceptExpressionForElementName(ResolvedTermExpression cte,
        ...
        for (NormalizedFact nf : facts) {
            ...
            if (nf.dim_segments != null && !nf.dim_segments.isEmpty()) {
                factsWithDim.add(nf);
            } else {
    ...
    private Boolean doesFactQualify(NormalizedFact fact,
        ...
        if (cTE.expression.usePositiveValuesOnly && fact.value < 0) {
            return false;
        }

        if (cTE.expression.useNegativeValuesOnly && fact.value > 0) {
            return false;
        }

        if (cTE.expression.reverseNegativeValues && fact.value < 0) {
            fact.value *= -1;
    ...
    private void populateFactsInFiscalHashWithDim(List<NormalizedFact> facts,
        ...
        for (NormalizedFact nf : facts) {
            ...
            String key = nf.getFiscalKeyWithEntity();
            if (nf.dim_segments != null && !nf.dim_segments.isEmpty()) {
                String dimKey = String.format("%s:%s", key, nf.dim_segments);
    ...
    private void buildTermResultsForConceptMatchFacts(
        ...
        for (List<NormalizedFact> factsInPeriod : factsByFYFQ.values()) {
            ...
            if (topFact.dim_segments == null || topFact.dim_segments.isEmpty()) {
                ...
                for (NormalizedFact qnf : validFacts) {
                    ...
                    resolvedExpression.append(String.format("%s %s {%s}",
                            prefix, qnf.dim_segments,
                            TermResult.decimalFormat.format(qnf.value)));
    ...
    private List<NormalizedFact> reduceDimensionalFactsToUseableFacts(
        ...
        for (NormalizedFact f : factsToConsider) {
            ...
            groupValuesByAxis.put(f.dimensionAxisKey, orig + f.value);
            ...
            for (List<NormalizedFact> factsInPeriod : factsByFYFQ.values()) {
                ...
                if (matchedDimItem != null) {
                    ...
                    tr.resolvedExpression = String.format("%s %s = %s",
                            matchedDimItem.elementName, dimMemberInfo,
                            TermResult.decimalFormat.format(tr.value));
                    addTermResult(tr);
    ...
    private void processExtendedCheck() {
        ...
        for (ResolvedTermExpression te : expressionMaps.get(TermExpressionTypeCode.ExtendedCheck)) {
            ...
            for (NormalizedFact nf : facts) {
                if (isProcessedByEarlierRank(nf.getCikString(), nf.FQ, nf.FY, te.expression.rank)) {
                ...
                //create termResult..
                TermResultsDoc tr = createTermResult(nf, te.expression, nf.elementName, null);
                if (tr == null) {
                    continue;
                }
                tr.resolvedExpression = String.format("%s = %s", nf.elementName, TermResult.decimalFormat.format(tr.value));
    ...
    private void processExtendedCheckWithDim() {
        ...
        for (ResolvedTermExpression te
            ...
            for (NormalizedFact dnf : facts) {
                ...
                if (isProcessedByEarlierRank(dnf.getCikString(), dnf.FQ, dnf.FY, te.expression.rank)) {
                    LOG.debug("skipping processing extended check as a better result already exists");
                    continue;
                }
                ...
                String dimMemberInfo = cte.GetValidDimensionMatchingInfo(dnf, dnf.elementName);

                if (dimMemberInfo == null) {
                    continue;
                }

                TermResultsDoc tr = createTermResult(dnf, te.expression, dnf.elementName,
                        dimMemberInfo);
                if (tr == null) {
                    continue;
                }
                tr.resolvedExpression = String.format("%s %s = %s", dnf.elementName, dimMemberInfo, TermResult.decimalFormat.format(tr.value));
                addTermResult(tr);
            }
    ...
    private Boolean canUseExtendedFact(NormalizedFact nf, Boolean isMonetaryRule) {
        ...
        return nf.elementType.equals("monetary") ? isMonetaryRule : !isMonetaryRule;
    }
    ...
    private void BuildFormulaResults(FormulaResolvedTermExpression formulaExp,
        ...
        for (String key : formulaData.resultsMap.keySet()) {
           ...
           // LOG.info("Base Formula ::: "+result.resolvedExpression);
            ...
            Entity entity = this.getEntityById(result.entityId);
            ...
            if (entity != null) {
                ...
                result.stockSymbol = entity.tradingSymbol;
