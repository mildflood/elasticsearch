...
//      "url" : "https://www.sec.gov/Archives/edgar/data/1750/000104746915006136/a2225345z10-k.htm",
	...
	private HashMap<String, AccuracyTestNotesDoc> getNotesDetails(String entityId, int fiscalYear) {
		...
		// List<AccuracyTestNotesDoc> results =
		// this.repository.findByEntityIdAndFYAndFQ(entityId, fiscalYear, "FY");

		try {
			List<AccuracyTestNotesDoc> docs = new ArrayList<AccuracyTestNotesDoc>();
			// Build Query
			BoolQueryBuilder boolQueryBuilder = new BoolQueryBuilder();
			boolQueryBuilder.must(new MatchQueryBuilder("entityId_s", entityId));
			boolQueryBuilder.must(new MatchQueryBuilder("fy_i", fiscalYear));
			boolQueryBuilder.must(new MatchQueryBuilder("fq_s", "FY"));
			...
			LOG.error("Elasticsearch failed in getNotesDetails: " + e.getMessage());
		}
		return ret;
	}
