...
public class TermResultService {
    ...
    public List<TermResultsDoc> findByTermId(String termId) {
        return resultsSolrRepo.findByTermIdAndMyDocType(termId, TermResultsDoc.TermResultsDocType.primaryTermResult.toString());
    }
    ...
    public List<TermResultsDoc> findBaseInfoByTermIdAndEntity(String termId, String entityId) {
        ...
        List<TermResultsDoc> dataResults1 = resultsSolrRepo.findByEntityIdAndTermIdAndMyDocType(entityId, termId,
                TermResultsDoc.TermResultsDocType.primaryTermResult.toString());
        ...
        List<TermResultsDoc> derivedZeros
                = resultsSolrRepo.findByEntityIdAndTermIdAndMyDocType(entityId, termId,
                        TermResultsDoc.TermResultsDocType.derivedZeroTermResult.toString());
        ...
        if (includeQuarterly) {
        	...
        	if (rank > 0 && rank != 99) {
                ...
                facetPage = resultsSolrRepo.findEntityFacetByTermIdEntitiesFyRangeRank(termId, entityMap,
                        minYearToCheck, maxYearToCheck, type, rank, new PageRequest(0, 20000));
            } else {

                facetPage = resultsSolrRepo.findEntityFacetByTermIdEntitiesFyRange(termId, entityMap,
                        minYearToCheck, maxYearToCheck, type, new PageRequest(0, 20000));
            }
        	...
        	if (rank > 0 && rank != 99) {
                ...
                facetPage = resultsSolrRepo.findEntityFacetByTermIdEntitiesFyRangeFQRank(termId, entityMap,
                        minYearToCheck, maxYearToCheck, "FY", type, rank, new PageRequest(0, 20000));
            } else {

                facetPage = resultsSolrRepo.findEntityFacetByTermIdEntitiesFyRangeFQ(termId, entityMap,
                        minYearToCheck, maxYearToCheck, "FY", type, new PageRequest(0, 20000));
            }
    ...
    public HashMap<Integer, List<String>> buildEntityCoverageByYear(
        ...
        String docType = useMissingCollection ? TermResultsDoc.TermResultsDocType.derivedZeroTermResult.toString()
                : TermResultsDoc.TermResultsDocType.primaryTermResult.toString();
...
//            }
    ...
    public void saveDerivedZeroData(Collection<TermResultsDerivedZero> col) {
        ...
        col.stream().map((item) -> {
            ...
            if (item.getHasTermResult()) {
                doc.myDocType = TermResultsDoc.TermResultsDocType.derivedZeroHasTermResult;
            }
            return doc;
        }).forEachOrdered((doc) -> {
            saveList.add(doc);
    ...
    public void saveDerivedNonZeroData(Collection<TermResultsDerivedNonZero> col) {
        ...
        col.stream().map((item) -> {
            ...
            //doc.verifiedNotes = item.getVerifiedNotes();
            doc.rank = 99;
            doc.myDocType = TermResultsDoc.TermResultsDocType.derivedNonZeroTermResult;
            return doc;
        }).forEachOrdered((doc) -> {
            saveList.add(doc);
        });
