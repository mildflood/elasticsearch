...
import gov.sec.idap.aop.String1;
...
//}]
	public HashMap<String, Double> getCompustatValues(String entityId, int fiscalYear,
		...
		try {
			...
			//select * from maxds.ViewC951B  where cik = 320193 and fyear = 2020	
			String fl = " * ";
//			if (vars.isPresent()) {
//				fl = String1.format("  gvkey, cik, datadate, curcd, conm, consol, cusip, datafmt, fdate, fyear, indfmt, popsrc, tic, %s, created, updated, c951b ",
//						vars.get());
//			}

			String sql = String1.format(" select %s from maxds.ViewC951B  where cik=%s and fyear=%s", fl, cik, ""+fiscalYear);
			LOG.trace(sql);
			List<Map<String, Object>> compustatData = jdbcTemplate.queryForList(sql);
		
			// only intereted in usd
			// "curcd" : "USD"
			for (Map<String, Object> dataByCur : compustatData) {
				...
				if (cur.equals("USD")) {
					...
					for (TermMapInformationDoc doc : compustatIds.values()) {
						...
						if (dataByCur.containsKey(key)) {
							...
							Double value = ((BigDecimal) dataByCur.get(key)).doubleValue();
...
//}
	public void setFilingInfo(AccuracyTestData data) {
		...
		try {
			...
			int cik = Integer.parseInt(data.cik);
			...
			//LinkedHashMap<String, Object> filingData = secApiService.getGenericJsonResponse(uri);//  select * from [Idap].[maxds].[ViewFiledUrl] where cik=320193 and fy=2020
			
			//String sql = String1.format(" select * from maxds.ViewFiledUrl  where cik=%s and fy=%s", data.cik, ""+ data.fiscalYear);
			String sql = "select * from maxds.ViewFiledUrl  where cik=" + cik + " and fy='"+ data.fiscalYear + "'"; 
			LOG.trace(sql);
			List<Map<String, Object>> filings = jdbcTemplate.queryForList(sql);
	...
	private List<AccuracyTestNotesDoc> getSearchResult(SearchResponse response) {
		SearchHit[] searchHit = response.getHits().getHits();
		List<AccuracyTestNotesDoc> docs = new ArrayList<>();
		for (SearchHit hit : searchHit) {
			docs.add(objectMapper.convertValue(hit.getSourceAsMap(), AccuracyTestNotesDoc.class));
		}
		return docs;
	}
	...
	private LinkedHashMap<String, Object> getGenericJsonResponse(String url, String... pathVariables) {
		...
		ParameterizedTypeReference<LinkedHashMap> responseType = new ParameterizedTypeReference<LinkedHashMap>() {
		};

		ResponseEntity<LinkedHashMap> restResponse = restTemplate.exchange(url, HttpMethod.GET, requestEntity,
