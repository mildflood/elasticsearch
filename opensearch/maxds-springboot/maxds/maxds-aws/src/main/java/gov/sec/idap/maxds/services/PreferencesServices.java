...
public class PreferencesServices {
	...
	public List<String> getAllIndustries() {
		...
		try {
			String sql = "select distinct  CONCAT(COALESCE(sic, 'null'),'-', industry) from norm2_ops.ViewSolrEntities";
			JdbcTemplate jdbcTemplate = util.getJdbcTemplate();
	...
	public List<String> getAllDivisionSectors() {
		...
		try {
			String sql = "select distinct CONCAT(COALESCE(division, 'null'),' -> ', sector) as divisionSector from norm2_ops.ViewSolrEntities where division is not NULL  order by divisionSector asc";
			JdbcTemplate jdbcTemplate = util.getJdbcTemplate();
	...
	public List<Map<String, Object>> getPeerDetails(String companyName) {
		...
		try {
			String sql = "select  *, CONCAT(COALESCE(sic, 'null'),'-', industry) as sicIndustry from norm2_ops.ViewSolrEntities where companyName= '" + companyName + "'";
			JdbcTemplate jdbcTemplate = util.getJdbcTemplate();
